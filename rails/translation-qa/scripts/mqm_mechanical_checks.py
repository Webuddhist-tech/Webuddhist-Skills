...
# Block IDs come in several shapes across the vaults (see CONVENTIONS.md 1 and 7):
#   ^583        flat, one segment          (verse_id_format: verse)
#   ^1-1        chapter-verse, two segments
#   ^1-2-3      three or more segments     (section / commentary node IDs)
#   ^I-4, ^a-2  letter zones               (front matter, colophons)
#   ^1-0a-1     letter sub-namespace under a heading's -0 slot
# The pattern is therefore one alphanumeric segment followed by any number of
# further `-segment` groups. The older two-segment-only pattern silently
# ignored single-segment and three-plus-segment IDs, so whole files scored as
# "no verses found" and their omissions went unreported.
...
def is_heading_id(vid):
    """A heading anchor is any ID whose final segment is the reserved `0` slot:
    ^1-0, ^1-2-0, ^I-0, ^a-0, and the bare ^0 book-title anchor."""
