#!/usr/bin/env python3
"""build_termbase.py — turn a decisions file into a target-language termbase, grade
file and review table (keyword-standardize Step 5). The language comes from the
decisions file's _meta.lang (zh, vi, …); the examples below are Chinese.
...
Usage:
    python3 build_termbase.py --decisions <kw>/zh/zh-decisions-general.json \\
...
from common import blocks, load_json, forms, clean_bo, source_label
...
def main(argv=None):
    ...
    ap.add_argument("--reference-prefix", default=None, help="block-ID prefix in the reference file (default: _meta.reference_prefix or zhc-)")
    ...
    LN = meta.get("language_name", {"zh": "Chinese", "vi": "Vietnamese", "hi": "Hindi", "ne": "Nepali", "mn": "Mongolian", "th": "Thai"}.get(L, L))
    if a.reference_prefix is None:
        a.reference_prefix = meta.get("reference_prefix", "zhc-")
    ...
    def row(e):
        return (f"| {e['bo']} | {e['en']} | **{e[L]}** | {e[f'{L}_source']} | {', '.join(e['verse_ids'])} | "
                f"{e[f'{L}_note']} |")
    H = f"| Tibetan | English | {LN} | Source | Verses | Why |\n|---|---|---|---|---|---|"
    ...
    md = ["---", f"title: {meta.get('title', LN + ' word list')}", f"language: {LN}" + (f" ({meta['script']})" if meta.get("script") else ""),
          f"lang_tag: {meta.get('lang_tag', L)}", "file_type: termbase-review", f"grade: {G}",
          f"termbase: {out_tb.name}", f"grade_file: {out_g.name}", f"decisions: {a.decisions.name}",
          f"built_by: keyword-standardize/scripts/build_termbase.py", f"entries: {len(tb)}",
          f"decided_flagged: {len(fl)}", "status: awaiting native-speaker review", "---", "",
          f"# {meta.get('title', LN + ' word list')}", "",
          f"The locked {LN} words for this text. Every {LN} translation step must use these words for these",
    ...
    if only:
        md += [f"**{LN}-only entries:** {', '.join(only)}.", ""]
